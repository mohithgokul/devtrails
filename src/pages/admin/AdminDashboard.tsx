...
import { adminKPIs, premiumVsClaims, adminAlerts } from '@/lib/adminMockData';
...
const kpiCards = [
    { label: 'Total Active Policies', value: adminKPIs.totalActivePolicies.toLocaleString(), change: adminKPIs.policyChange, icon: FileText, color: 'text-primary' },
    { label: 'Premiums Collected', value: formatCurrency(adminKPIs.totalPremiums), change: adminKPIs.premiumChange, icon: Banknote, color: 'text-secondary' },
    { label: 'Claims Paid', value: formatCurrency(adminKPIs.totalClaimsPaid), change: adminKPIs.claimsChange, icon: ClipboardList, color: 'text-accent' },
    { label: 'Overall Loss Ratio', value: `${adminKPIs.overallLossRatio}%`, change: null, icon: TrendingUp, color: adminKPIs.overallLossRatio > 80 ? 'text-destructive' : adminKPIs.overallLossRatio > 60 ? 'text-accent' : 'text-secondary' },
];
                ...
                <div className="bg-white rounded-xl p-5 border border-border/50 shadow-sm animate-fade-in-up" style={{ animationDelay: '400ms' }}>
                    ...
                    <div className="space-y-3">
                        {adminAlerts.map((alert, i) => (
                            <div key={i} className={cn('p-3 rounded-lg border-l-4 text-xs leading-relaxed text-foreground', alertStyles[alert.type])}>
